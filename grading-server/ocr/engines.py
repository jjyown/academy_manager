...
특히 아래에 더 집중하세요:
- 객관식: 동그라미 표시의 물리적 위치를 기준으로 보기 번호 판단!
  → 보기 배치(①②③ / ④⑤)에서 동그라미가 어느 칸에 있는지 위치로 확인
  → 동그라미 안의 숫자만 보지 말고, 주변 인쇄된 번호와 대조
  → ②와 ⑤ 혼동 주의! 위치가 윗줄이면 ①②③, 아랫줄이면 ④⑤
...
[객관식 - 가장 주의! 오류가 가장 많은 영역]
- 학생이 인쇄된 보기 번호(①②③④⑤) 위에 동그라미/체크/밑줄로 선택 표시
- 반드시 원형 숫자로 기록: ①②③④⑤ (일반 숫자 "1","2" 금지)

★★ 객관식 위치 확인법 (반드시 따르기) ★★
1. 먼저 해당 문제의 보기 ①②③④⑤가 어떤 배열(가로/세로)로 배치되어 있는지 파악
2. 학생의 동그라미/체크 표시가 어느 위치에 있는지 확인
3. 그 위치에 인쇄된 보기 번호가 몇 번인지 읽기
4. 혼동하기 쉬운 번호 주의: ②와 ⑤는 동그라미 안에서 매우 비슷함!
   - ② = 2 (획이 위에서 시작, 아래로 꺾임)
   - ⑤ = 5 (획이 위에서 시작, 중간에 튀어나옴)
   - 확실하지 않으면 보기 배치의 위치(왼쪽/오른쪽, 위/아래)로 판단
5. 보기가 2행으로 배열된 경우(예: ① ② ③ / ④ ⑤):
   - 윗줄 왼쪽=①, 윗줄 가운데=②, 윗줄 오른쪽=③
   - 아랫줄 왼쪽=④, 아랫줄 오른쪽=⑤
