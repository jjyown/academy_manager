...
- [ ] 테스트 점수 원격 동기화는 코드 반영 완료 상태이나, `STUDENT_TEST_SCORE_SETUP.sql`을 Supabase SQL Editor에서 실행해야 실운영 동기화가 활성화됨
- [ ] 학생 스키마 확장 컬럼(`guardian_name`, `enrollment_start_date`, `enrollment_end_date`)은 코드 반영 완료 상태이나, `STUDENT_SCHEMA_UPDATE.sql`을 Supabase SQL Editor에서 실행해야 완전 동기화됨
...
- 학생관리 최신 반영(이번 작업):
  - 학생등록 저장 시 필수검증: 이름/등록일/연락처 1개 이상
  - 연락처 형식검증: 휴대전화 형식(01x, 10~11자리)
  - 동일 학생 의심 중복 차단: 이름+학년+연락처 일치 시 저장 중단
  - 일정 삭제 모달: 삭제 예정 일정 건수 미리보기 추가
  - 학생 이력 모달: 테스트 점수 입력/월별 변화 추이(평균/변화/목록) 추가
  - 등록/수정 폼 확장: 재원 상태 + 시작일/종료일 + 보호자 이름 입력
  - 저장 검증 확장: 시작일 필수, 휴원/퇴원 종료일 필수, 기간 역전 차단
  - DB 스키마 확장 SQL 추가: `STUDENT_SCHEMA_UPDATE.sql`
- 재개 시작 순서(고정):
...
## 다음 작업 우선순위 (2026-03-04, 전체 시스템 보정판)
1) QR 긴급출석(임시출석) 도입 - 일정 누락 시 현장 출석 흐름 복구
2) 임시출석 확정 워크플로우 - 확정 대기함/수동 확정/사유 기록
3) 일정 누락 사전 예방 - 당일 누락 알림 + QR 진입 전 경고 + 자동 생성 제안
4) 학생 상세 통합 화면 액션화 - 조회 중심에서 조치 중심으로 확장
5) 테스트 점수 원격 동기화 운영 검증(실데이터 저장/조회/삭제 + 폴백 시나리오)
6) 학생 스키마 확장 SQL 운영 반영 검증(`STUDENT_SCHEMA_UPDATE.sql`)
7) 테스트 점수 SQL 운영 반영 검증(`STUDENT_TEST_SCORE_SETUP.sql`)
