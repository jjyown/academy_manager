...
| 2026-05-18 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | .claude/agents/academy-developer.md, .claude/agents/academy-reviewer.md |
| 2026-05-18 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | CLAUDE.md, docs/CLAUDE_USAGE_GUIDE.md, docs/DISCUSSIONS.md |
| 2026-05-18 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | docs/CLAUDE_LEARNING_NOTES.md, docs/CLAUDE_USAGE_GUIDE.md |
| 2026-05-18 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | CLAUDE.md |
| 2026-05-18 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | CLAUDE.md |
| 2026-05-17 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/routers/results.py |
| 2026-05-17 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/routers/grading.py |
| 2026-05-16 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading/index.html |
| 2026-05-16 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/routers/grading.py |
| 2026-05-15 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/file_utils.py, grading-server/routers/grading.py, grading-server/routers/results.py |
| 2026-05-14 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/routers/results.py |
| 2026-05-14 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | supabase/functions/upload-homework/index.ts |
| 2026-05-14 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | homework/index.html, migrations/0044_hw_submissions_unique_idx_20260514.sql |
| 2026-05-14 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/routers/homework_submissions.py |
| 2026-05-14 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/routers/results.py, grading/index.html |
| 2026-05-14 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | homework/index.html, supabase/functions/upload-homework/index.ts |
| 2026-05-14 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/tests/test_smoke_stage1.py |
| 2026-05-14 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/routers/grading.py, homework/index.html, migrations/0043_homework_files_json_20260514.sql, supabase/functions/upload-homework/index.ts |
| 2026-05-14 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | supabase/functions/upload-homework/index.ts |
| 2026-05-14 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | homework/index.html, supabase/functions/upload-homework/index.ts |
| 2026-05-14 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | supabase/functions/upload-homework/index.ts |
| 2026-05-14 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | homework/index.html, js/payment.js |
| 2026-05-14 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/tests/test_smoke_pdf_parser.py |
| 2026-05-14 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/grading/pdf_parser.py, grading-server/tests/test_smoke_pdf_parser.py |
| 2026-05-13 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | .claude/CLAUDE.backend.md, .claude/CLAUDE.frontend.md, .gitignore, CLAUDE.md |
...
| 2026-05-12 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/routers/answer_keys.py |
| 2026-05-12 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/routers/answer_keys.py |
| 2026-05-12 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/routers/answer_keys.py |
| 2026-05-12 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/grading/pdf_parser.py |
| 2026-05-12 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/grading/pdf_parser.py |
...
| 2026-05-12 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/routers/answer_keys.py |
| 2026-05-12 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/grading/pdf_parser.py |
| 2026-05-12 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | index.html, js/payment.js |
| 2026-05-12 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js, database.js, supabase/functions/investigate-school-calendar/index.ts |
| 2026-05-12 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js, database.js, migrations/0042_school_calendar_files_20260512.sql, style.css |
| 2026-05-12 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | js/payment.js, style.css |
| 2026-05-12 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js, style.css |
| 2026-05-12 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | index.html, js/payment.js, style.css |
| 2026-05-12 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | style.css |
| 2026-05-12 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js, script.js |
...
| 2026-05-11 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | supabase/functions/investigate-school-calendar/index.ts |
| 2026-05-11 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js, database.js, style.css, supabase/functions/investigate-school-calendar/index.ts |
| 2026-05-11 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | style.css |
| 2026-05-11 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | style.css |
| 2026-05-11 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js, database.js, homework/index.html, js/academic-events-client.js, migrations/0041_school_calendar_overrides_20260511.sql 외 2개 |
| 2026-05-11 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | index.html, js/payment.js, style.css |
| 2026-05-11 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading/index.html |
| 2026-05-11 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | index.html, js/payment.js, style.css |
| 2026-05-11 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/routers/answer_keys.py, grading/index.html, migrations/0040_answer_keys_source_type_exam_20260511.sql |
| 2026-05-11 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | script.js |
...
| 2026-05-10 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | .gitignore, migrations/0034_cleanup_expired_reset_codes_20260510.sql, qa-artifacts/quick-recheck-report.json, qa-artifacts/report.json, qa-artifacts/runtime-agent-verify-long.json 외 2개 |
| 2026-05-10 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/ocr/mathpix.py |
| 2026-05-10 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/.env.example, grading-server/ocr/mathpix.py, grading-server/routers/mathpix_status.py |
| 2026-05-10 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/ocr/mathpix.py, grading-server/routers/mathpix_status.py |
| 2026-05-10 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/auth.py |
| 2026-05-10 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/.env.example, grading-server/config.py, grading-server/grading/pdf_parser.py, grading-server/main.py, grading-server/ocr/mathpix.py 외 1개 |
| 2026-05-10 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | docs/architecture-images/01-system-overview.png, docs/architecture-images/02-eval-pipeline.png, docs/architecture-images/03-er-diagram.png, docs/architecture-images/04-eval-sequence.png, docs/architecture.md |
| 2026-05-10 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | index.html, script.js, style.css, supabase/functions/generate-student-eval-report/index.ts |
| 2026-05-10 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | script.js |
| 2026-05-10 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | database.js, migrations/0033_student_eval_image_url_20260510.sql, parent-portal/index.html, parent-portal/report.js, script.js |
| 2026-05-10 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | supabase/functions/collect-admissions-knowledge/index.ts, supabase/functions/generate-student-eval-report/index.ts |
| 2026-05-10 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | js/academic-events-client.js |
| 2026-05-10 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | homework/index.html, js/academic-events-client.js, parent-portal/index.html, parent-portal/report.js |
| 2026-05-10 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js, homework/index.html, index.html, js/academic-events-client.js, parent-portal/index.html 외 2개 |
| 2026-05-10 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | index.html, script.js, style.css |
...
| 2026-05-10 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | supabase/functions/generate-student-eval-report/index.ts |
| 2026-05-10 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | parent-portal/index.html, parent-portal/report.js |
| 2026-05-10 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | database.js, script.js |
| 2026-05-10 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js, style.css |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js, style.css |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js, index.html, style.css |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js, style.css |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | style.css |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js, index.html, script.js, style.css |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js, style.css |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js, style.css |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js, index.html, script.js, style.css |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | supabase-config.js |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | script.js |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | script.js |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | script.js |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | vercel.json |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | index.html, script.js, style.css |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js, script.js, style.css |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js, style.css |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js, style.css |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js, style.css |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | academic-calendar.js, index.html, script.js, style.css |
...
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | script.js |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | index.html, script.js, style.css |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | homework/index.html |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/grading/confirm_drive_publish.py, grading-server/integrations/drive.py, grading-server/routers/grading.py, grading-server/routers/results.py, grading/index.html |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading-server/grading/grader.py, grading-server/integrations/supabase_client.py, grading-server/routers/results.py, grading/index.html |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | migrations/0030_drop_answer_keys_broad_read_20260509.sql |
| 2026-05-09 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | migrations/0027_revoke_trigger_func_execute_20260509.sql, migrations/0029_index_cleanup_20260509.sql |
...
| 2026-04-06 | **문항 그리드 빈 안내 레이아웃**: `#detail-grid.question-grid`가 `minmax(50px,1fr)`라 단일 자식 `div`가 50px 열에만 들어가 한 글자씩 세로로 보임 → `.question-grid-empty { grid-column: 1 / -1 }` | 빈 문항 메시지 가독성 | `grading/index.html` |
| 2026-04-06 | **`GET /api/results` 500**: `homework_submissions`에 `zip_drive_id` 컬럼이 없는 DB에서 select에 넣으면 PostgREST 오류 → **실제 컬럼만** 조회(`central_drive_file_*`, `drive_file_id`) · 확정 ZIP 조회 동일 | 운영 스키마와 API 파라미터명 혼동 제거 | `results.py`, `confirm_drive_publish.py` |
...
| 2026-03-31 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | grading/index.html |
| 2026-03-31 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | docs/checklist.md, docs/context.md, docs/plan.md, grading-server/routers/results.py, grading/index.html |
| 2026-03-31 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | docs/checklist.md, docs/context.md, docs/plan.md, grading/index.html |
...
| 2026-03-30 | **채점관리 초기 로드 AbortError**: 세션 복구 시 `loadTeacherList`+`loadAllData` 병렬로 Railway에 요청이 몰려 cold start·타임아웃 시 `AbortError` 다발 가능 → **순차** `loadTeacherList` 후 `loadAllData`. 과제·교재 로드 catch에 AbortError 안내 문구 | 빈 달력·빈 배정 완화 | `grading/index.html` |
| 2026-03-30 | **숙제 제출 Edge 401**: `upload-homework`가 Bearer JWT(원장) 또는 Form `student_code`(DB `students.student_code`와 정규화 비교 통과)만 인정. `homework/index.html`은 `currentStudent.student_code`가 비어있으면(또는 누락되면) 학생 포털에서 입력한 인증코드(정규화값)를 `student_code`에 fallback으로 재전송하도록 보강했음. **대응**: DevTools에서 `upload-homework` 요청 FormData에 `student_code`가 실제로 포함되는지와 Supabase `students.student_code`/정규화 일치 확인, 필요 시 Edge 재배포 병행 | 학생 숙제 제출 복구(재발 확인) | `upload-homework/index.ts`, `homework/index.html` |
| 2026-03-30 | **현재 401 메시지 확정**: Edge 응답 본문에 “원장 계정 로그인 또는 학생 인증코드(student_code)가 필요”가 그대로 표시됨. 즉, `jwtOwnerId`가 매칭되지 않았고/또는 Form `student_code`가 누락/불일치 상태. 다음 단계는 DevTools의 `upload-homework` 요청에서 실제 `student_code` 필드 포함 여부를 재확인하고, Supabase `students.status=active` 및 `students.student_code` 값을 채우는 것으로 대응 | 원인 분기 확정(Authorization vs student_code) | `upload-homework/index.ts`, `homework/index.html` |
| 2026-03-30 | **`due_time` 항상 NULL(컬럼은 있음) 운영 판별**: Supabase에 `due_time` 컬럼이 보이면 SQL 마이그레이션은 된 것. 값이 한 건도 안 들어가면 **Railway 채점 서버가 `due_time` Form을 처리하는 빌드로 배포됐는지**를 우선 의심(구버전은 필드를 무시). 브라우저 Network에서 `POST/PUT` FormData에 `due_time` 포함 여부로 클라이언트를 가름 | 스키마 OK + 앱 NULL = 배포/경로 이슈 분리 | 운영 점검 |
| 2026-03-30 | **채점 페이지 기본 API URL**: `grading/index.html`에서 `grading_server_url` 미설정 시 **항상** `DEFAULT_GRADING_SERVER_URL`(Railway) 사용 — 이전에는 localhost만 기본값·그 외는 빈 문자열이라 상대 `/api`로 깨질 수 있었음 | 과제 API가 엉뚱한 오리진으로 가는 경우 완화 | `grading/index.html` |
| 2026-03-30 | **과제 `due_time` UI 저장값 확정**: `input type="time"`이 포커스가 남은 채 저장되면 `.value`가 빈 문자열로 나가 Supabase에 NULL로 들어가는 사례 대비 — 저장 직전 `commitAssignDuePickers`(blur + rAF), `oninput`/`onchange`로 `dataset.committedTime`·`committedDate` 유지, `formatDueTimeForInput`은 `14:30:00` 등 문자열 전체에서 HH:MM 추출 | 성공 토스트 후에도 DB·새로고침 시 마감 시각 누락 방지 | `grading/index.html` |
| 2026-03-30 | **과제 마감 시각(선생님 지정)**: `grading_assignments.due_time`(TIME, NULL) + `due_date`. 배정·수정 UI·`POST`/`PUT /api/assignments`에 반영. 마감 판정(O/△/X 등)은 추후 `due_date`+`due_time` 합산과 제출 시각 비교로 통일 예정 | 일정 자동 정각 대신 운영에서 명시적 마감 | `SUPABASE_GRADING_ASSIGNMENTS_DUE_TIME_20260330.sql`, `assignments.py`, `grading/index.html` |
| 2026-03-30 | **due_time 저장 보강**: Supabase `insert`/`update`에 `.select("*")`로 저장 직후 행 반환·실패 시 예외 전달. `answer_key_id` 등 Form을 `int \| None`으로 정리. `_norm_due_time`에 전각 콜론·`H:MM` 형식 보강. UI는 실패 시 `detail` 토스트 | 저장은 됐는데 응답 본문에 컬럼이 안 보이거나 422가 숨겨지는 경우 완화 | `supabase_client.py`, `assignments.py`, `grading/index.html` |
| 2026-03-30 | **Drive 중앙 루트 단일화(숙제 관리 우선) + HML 미리보기**: My Drive 최상위 폴더를 조회하되 `resolve_central_root_folder_id`가 검색/생성 시 **`숙제 관리`를 최우선**으로 사용하며, 없으면 레거시/환경값 재사용 없이 **`숙제 관리`를 새로 생성**. HML은 원문 페이지 래스터 없음 → `build_hml_answer_preview_images`로 정답·해설 요약 JPEG 생성 후 `page_images_json`과 동일 경로 업로드. 또한 프론트/백엔드에서 `.hwp`도 HML 계열로 판별해 미리보기 누락을 줄임. 추가로 이번 로그에서 `페이지 이미지: 0장`이 관측되어, Pillow 미설치/미이미지 생성 실패 시에도 placeholder JPEG로 최소 1장 생성하도록 보강. 그리고 교재 페이지 이미지는 `grade_level` 폴더(예: `고3`) 우선 업로드 후 없으면 레거시(`교재 페이지 이미지`) 폴백으로 정렬 | `과제 관리`/`숙제 관리` 이중 루트·HML 이미지 누락(또는 ‘다른 루트에 업로드’로 인한 미인지) + 학년 폴더 기대 불일치 완화 | `drive.py`, `config.py`, `hml_parser.py`, `answer_keys.py` |
| 2026-03-30 | **Drive 레거시 루트(`과제 관리`) 통합 요청 대응 방향 확정 필요**: 이미 생성된 legacy 루트는 “신규 업로드 경로 고정”만으로는 정리되지 않으므로, 운영에서 `과제 관리` 하위(교재/제출 원본/채점 결과)를 `숙제 관리`로 병합/이관할지(파일 ID 유지 가능 방식 vs 수동 병합+검증) 결정 필요 | 사용자 탐색 혼선 제거(Drive 2개 루트) | 운영절차/추후 반영 |
...
| 2026-03-25 | **테스트 점수 Supabase 저장 + 그래프 탭**: `saveStudentTestScore`는 **`await _resolveOwnerUserId()`** 로 `owner_user_id`를 세션과 일치(RLS 통과). 평가 모달에 **그래프** 탭을 두어 **기준월~역산 1~12개월** 구간을 `getStudentTestScoresByDateRange`로 조회·대형 시각화(`wide`). 점수 탭은 입력·요약·목록만 | 작은 그래프 영역 불만·원격 저장 실패(오너 ID 불일치) 동시 대응 | `database.js`, `script.js`, `index.html`, `style.css` |
| 2026-03-25 | **점수 탭 시각화**: `buildTestScoreVisualizationHtml` — 만점 대비 %의 **SVG 라인 추이**(2건+), 1건은 점, **막대 그래프**에 %·일·시험명(축약), 데이터는 **시험일 순** 통일 | 성적 비교는 만점 대비 비율이 교육 현장에서 가장 흔한 스케일이며, 빈 달은 입력 유도 카피로 UX 마무리 | `script.js`, `style.css` |
| 2026-03-25 | **학생목록 「평가」모달**: `student-eval-modal` — **기록** 탭=수업관리 월별 개인·공유 메모 타임라인, **점수** 탭=테스트 점수(기존 `renderTestScoreSection`), 하단 **종합평가** 고정. 학생 카드 버튼을 `이력`+`점수`에서 **「평가」** 하나로 통합 | 메모 열람·성적 입력·월말 평가를 한 화면에서 끝내기 위해. 기록 탭의 의미를 “수업관리 입력의 월별 모음”으로 명시 | `index.html`, `style.css`, `script.js` |
| 2026-03-25 | **수업관리「이번달 기록」= 메모만 / 종합평가 숨김**: `openHistoryModal(true)`로 `#eval-section` 비표시·종합평가 로드 생략 | 수업 중에는 월별 메모 확인과 평가 작성을 분리해 맥락 혼선을 줄이기 위해 | `index.html`, `script.js` |
| 2026-03-24 | **재석확인 시간키 정규화(`HH:MM`) 강제**: 스누즈/대기큐/미스캔 알림 키를 `normalizeAttendanceTimeKey`로 통일해 `HH:MM` vs `HH:MM:SS` 혼재 시 중복 알림·스누즈 무시를 방지 | 사용자 보고(5분 스누즈 후 1분 내 재노출)는 시간키 포맷 불일치가 남아있을 때 재현 가능성이 있어 키 체계 통일이 필요 | `qr-attendance.js` |
| 2026-03-24 | **결제 증빙 업로드+AI 추출 모달 용어/옵션 통일**: 안내 문구를 `카드/계좌이체/QR코드` 기준으로 변경하고 증빙 유형을 `카드결제 화면/계좌이체 내역/QR결제 내역/기타`로 재정의, 기본값을 `카드결제 화면`으로 조정 | 메인 수납 모달과 AI 업로드 모달의 분류 체계 불일치를 제거해 입력 혼선을 줄이기 위해 | `index.html`, `js/payment.js` |
| 2026-03-24 | **재석확인은 “현재 시간표 기준”으로만 표시**: 일정 변경 직후 5분 스누즈/구시간 알림 잔류를 막기 위해, 재석확인 큐·스누즈·타이머를 시간표 기준으로 즉시 정리하고 미스캔 재평가 키도 함께 정리 | 사용자 이슈(18:30→19:00 변경 후에도 18:30 재알림, 5분 스누즈 전 재노출)는 stale 큐/스누즈 데이터가 원인 | `qr-attendance.js`, `script.js` |
| 2026-03-24 | **수납관리 대사(차이) 영역 제거**: 좌측 요약의 수단별 외부 실합계 입력/차이 카드와 `장부와 실제 합계 차이` 경고 박스를 UI에서 삭제 | 운영에서 해당 대사 기능을 사용하지 않아 화면 복잡도와 혼선을 줄이기 위해 | `index.html` |
| 2026-03-24 | **수납 결제수단 옵션/용어 통일**: 메인 원장 모달과 AI 검토 모달 모두 `카드/계좌이체/QR코드`만 사용하고 결제경로 입력은 제거. 내부 집계는 `계좌이체→통장`, `카드/QR코드→기타`로 자동 파생, 거래확인번호 placeholder는 결제수단별 예시로 명확화 | 화면마다 항목이 다르면 입력 실수와 운영 혼선이 반복되므로 동일 스키마로 통일해야 함 | `index.html`, `js/payment.js` |
...
| 2026-03-23 | **기간 삭제 = owner 소속 전 선생님 `teacher_id` + 다른 선생님 일정 확인 모달**: 스키마상 일정 행의 `teacher_id`는 「어느 선생님 시간표 칸」이며, 기간 삭제를 현재 선생님만으로 제한하면 타 선생님이 등록한 일정(내 담당 학생이라도)이 빠진다. 대응: `deleteSchedulesByOwnerRange`·`fetchDistinctStudentIdsFromSchedulesInRangeForOwner` 등(`database.js`). 실행은 로컬 `teacherScheduleData` **전 선생님 키** 루프 + 출석은 `collectScheduleSlotsByRangeFromDbAllTeachers`. 기간 내 DB에 **현재 선생님 외 `teacher_id`가 있으면** `hasOtherTeacherSchedulesInPeriodDeleteScope` → `showConfirm` 후 담당 외 모달·최종 확인. **전문가(프론트/운영)**: 다중 강사 환경에서는 삭제 영향 범위를 DB의 `teacher_id` 기준으로 밝히고, 확인 단계를 분리하는 편이 오삭제 분쟁을 줄인다 | 한 선생님 화면에서 기간 삭제 시 타 선생님 일정 미삭제·무단 삭제 인지 | `database.js`, `script.js`, `index.html` |
| 2026-03-23 | **기간 삭제「전체」범위 집계 = 로컬 키만이 아님**: `teacherScheduleData[teacherId]` 키는 담당 학생 위주로 채워지는 경우가 많아, **담당 외 학생 일정은 DB(`schedules`)에만 있고 로컬 버킷에 학생 키가 없으면** 미리보기·`expectedCount`가 0으로 나왔다. 대응: `getPeriodDeleteMergedStats`로 **학생 ID = 로컬 키 ∪ 기간 내 DB `student_id`**, 건수는 **로컬 합계와 DB 행 수(`count`) 중 큰 값**(`Math.max`). `executePeriodDelete`·`updatePeriodDeletePreview`(전체 범위·디바운스·모달 오픈 시 갱신) 동일 기준. **전문가(프론트/데이터)**: 집계는 **화면에 로드된 객체 구조가 아니라 삭제 실행과 같은 소스(로컬∪DB)** 로 맞추는 것이 운영 신뢰에 유리하다 | 「전체 학생」인데 삭제 예정 0건·담당 외 일정 누락 | `script.js` |
| 2026-03-23 | **기간 삭제 담당 외 모달 오탐 + 푸터 UI**: 담당 판별이 `getAssignedTeacherId` 빈 값이면 전부 「담당 아님」으로 보이던 경로를 끊기 위해, 담당 **외** 확인은 **`studentHasDifferentPrimaryTeacherThan`** 만 사용(명시적으로 **다른** 원생 담당이 있을 때만 true; 담당 정보가 비어 있으면 담당 외로 보지 않음). `studentPrimaryAssignedMatchesTeacher`는 담당만 삭제 필터용으로 `currentTeacherStudents` 포함 등 보강 유지. 모달 푸터는 `#period-del-nonassign-modal`·`#period-del-admin-modal`에 `period-del-footer*` + 관리자 행에서 **전역 `btn-delete-action { width:100% }`로 취소 버튼이 찌그러지던 문제**를 `style.css`에서 완화 | 담당 학생만 있는데도 담당 외 모달·취소 버튼 세로 글자 깨짐 | `script.js`, `index.html`, `style.css` |
| 2026-03-23 | **일정 날짜 문자열 정규화(2월 삭제 실패)**: 로컬·레거시에 `2026-2-5`처럼 월·일 한 자리 키가 있으면 **문자열 비교**로 `dateStr < start` / `dateStr > end`가 깨져 2월 구간만 집계·로컬 삭제 루프에서 제외될 수 있음(`2026-2-5` > `2026-02-28`). 단건 삭제는 `eq('schedule_date', 비정규)`로 DB 0건. 대응: `normalizeScheduleDateKey`(database.js)·`normalizeScheduleDateKeyLocal`·`normalizeAllTeacherScheduleDataDateKeys`, 삭제/조회/저장 경로에 적용 | 2월만 안 지워짐·새로고침 시 복귀 | `database.js`, `script.js` |
| 2026-03-23 | **일정 단건 삭제 → Supabase**: `deleteScheduleFromDatabase`에 `owner_user_id` 조건 추가(RLS·다른 테넌트와 정합). `start_time`은 `HH:MM`/`HH:MM:SS` 변형을 `.in`으로 삭제. `deleteSingleSchedule`은 **DB 삭제 성공 후**에만 로컬·토스트 처리(이전에는 백그라운드로 UI만 먼저 지움) | 사용자 화면에서는 지워졌는데 DB에는 남는 불일치 | `database.js`, `script.js` |
| 2026-03-23 | **전체 선생님 보기 + 기간 삭제**: `timetableScope==='all'`일 때 캘린더는 `teacherScheduleData`의 **모든 선생님 키**를 합산한다. 기간 삭제 후 `loadTeacherScheduleData(현재)`만 호출하면 **타 선생님 메모리 버킷**에 예전 월(예: 2월) 일정이 남아 화면에 그대로 보였다. 삭제·일괄 삭제 성공 후 `reloadScheduleDataAfterOwnerMutation()` → `loadAllTeachersScheduleData()`로 동기화. 추가로 `loadAllTeachersScheduleData`는 DB에 행이 없는 알려진 선생님도 `otherTeachers[tid]||{}`로 **빈 버킷 갱신**해 전부 삭제 후 stale 잔류를 제거 | “삭제했는데 캘린더만 안 없어짐” (DB는 지워졌을 수 있음) | `script.js` |
| 2026-03-23 | **기간 삭제 담당 외 학생 UX**: 기간·DB·로컬에서 **실제 일정이 있는 학생**만 대상으로 원생 담당 여부를 판별(`checkSet` 정교화). 담당 외가 섞이면 안내 모달 → **삭제하기**는 원장 PIN(`verifyTeacherPinWithServer`·`requireAdmin`) 후 전체 삭제, **아니요**는 확인/취소 후 **담당 학생만** `deleteSchedulesByRange` 루프. 닫기 시 `__periodDeleteCtx` 초기화 | 잘못된 `filter(... \|\| true)` 제거·오탐 감소, 권한 분리 | `script.js`, `index.html` |
| 2026-03-23 | **기간 일정 삭제**: Supabase 반영 전에 UI가 성공 처리되던 경로를 `await`로 묶음. 전체 범위는 `deleteSchedulesByTeacherRange` 단일 호출로 통일해 중복 삭제 호출 제거 | 삭제 성공 토스트 후 DB 재로드로 일정이 되살아 보이는 현상 | `script.js`, `database.js` |
| 2026-03-23 | 학부모 포털 **인증시간**: `resolveParentPortalAuthIso`를 출석 이력 `authIso`와 동일 분기로 고정. 원장이 `auth_time`을 수정하면 그 값이 그대로 표시되고, `check_in_time`은 번호 인증 등에만 사용(무조건 폴백 시 처리시간과 혼동) | 이력 UI와 데이터 소스 불일치 | `parent-portal/report.js` |
| 2026-03-23 | 학부모 포털 **출결 메모**: 선생님이 이력 카드 하단에 저장한 `memo`는 **지각·결석·보강·기타**일 때만 하단 박스에 노출. **보강** 등에 사유만 적은 경우(예: 메모에 「지각」)도 학부모가 확인 가능 | 일반 출석(`present`)은 내부 메모 혼입 가능성으로 제외 | `parent-portal/report.js` |
| 2026-03-23 | 학부모 포털 **지각** 행: `align-items: flex-end`로 **수업·인증**을 출석 카드와 같이 **아래 줄에서 나란히** 맞춤. `flex-start`였을 때는 오른쪽 열 상단(지각 분)과 왼쪽 수업이 한 높이로 붙어 라인이 어긋남 | 2열 중 오른쪽만 2줄(지각분+인증)일 때 크로스축 정렬 필요 | `parent-portal/index.html` |
| 2026-03-23 | 학부모 포털 **지각** 출결 상세: 수업·인증은 **가로 한 줄**(`att-meta-late-row`, `flex-wrap: nowrap`)로 묶어 모바일에서 줄바꿈 지그재그 방지. 인증 열 안에서는 **N분 지각**을 인증 시각 **위**에 유지 | `att-detail-meta`가 `wrap`이라 두 블록이 세로로 갈라질 수 있음 | `parent-portal/report.js`, `parent-portal/index.html` |
| 2026-03-23 | 학부모 포털 **출결 일별 상세**는 메인과 동일하게 **인증시각**을 `auth_time` 우선·레거시 `qr_scan_time`/`check_in_time` 폴백으로 표시. **지각**만 수업 시작(날짜+`scheduled_time`) 대비 **지연 분**을 인증 라벨 위에 표시. **결석**은 인증 미표시 | 학부모에게 처리방식(QR/선생님 확인) 대신 통일된 **인증시간**·지각량 전달 | `parent-portal/report.js`, `parent-portal/index.html` |
| 2026-03-23 | 출석 이력 **처리시간·인증시간**: 「수정」버튼 제거, 값 변경 후 **Enter**로만 저장(`saveAttendanceHistoryTimeField`). 초기값과 동일하면 저장 생략 | UI 단순화·요청 반영 | `qr-attendance.js` |
| 2026-03-23 | **채점 API Docker**: 루트 `docker-compose.yml`로 `grading-server` 이미지 빌드·실행, `Dockerfile`에 `PORT` 셸 확장·`curl`·헬스체크·`.dockerignore` 정리 — 로컬과 클라우드(동일 이미지·환경변수) 전환 경로 확보 | 배포 전 로컬에서 컨테이너 검증·클라우드 시크릿만 주입하면 되게 함 | `docker-compose.yml`, `grading-server/Dockerfile`, `grading-server/.dockerignore`, `README.md` |
...
| 2026-03-22 | 전문가 합의(UI/UX): 출석 이력 카드에서 **처리시간·인증시간**을 세로로 쌓지 않고 **같은 행에** `라벨 : 입력` 형태로 나란히 표시, datetime 입력 폭 축소로 가독성 확보 | 사용자 요청 | `qr-attendance.js` |
| 2026-03-22 | 전문가 합의(UI/UX): 처리시간·인증시간은 **수정한 필드만「수정」버튼 활성화** 후 클릭 시 DB 반영 — `datetime-local` 즉시 저장·전체 리렌더로 인한 입력 방해 완화 | 사용자 요청 | `qr-attendance.js` `updateAttHistoryTimeApplyState` |
...
| 2026-03-22 | 전문가 합의(교육운영/프론트): 선생님 선택 QR은 **입장 PIN과 동일한 서버 검증** 후에만 스캔을 연다 — 태블릿에서 **메인 일정까지 진입하지 않고** 출석만 처리(학생이 해당 선생님 일정을 만질 위험 감소). 닫기는 기존처럼 **종료 PIN** 후 `TEACHER_SELECT` 복귀. **가시성**: `#qr-scan-page` **z-index 90000**(토스트보다 낮음)·inline 보강·선생님 선택 **일시 z-index 하향** | 운영·태블릿 동선 | `qr-attendance.js`, `style.css`, `index.html` |
| 2026-03-22 | 전문가 합의(UX/운영): **선생님 선택** 상단 QR은 **입장과 동일한 `verifyTeacherPinWithServer`(드롭다운+비밀번호)** 후 전체화면 진입, 닫기는 **종료 PIN** 후 `TEACHER_SELECT` 복귀. 미입장·`qrAttendanceTeacherIdOverride`·일정 스냅샷·readerWidth 대응은 `openQRScanPage` 기존 로직 유지 | 태블릿에서 메인 일정 노출 최소화·운영 동선 | `index.html`, `qr-attendance.js` |
| 2026-03-22 | **인지·후속(성능/아키텍처)**: 학생·일정 증가에 대비해 `attendance_records` **단건 반복 조회(N+1)** 가능성을 문서에 고정한다. 개선 방향은 **날짜·owner 묶음 조회 + 메모리 Map 판별**, 우선순위는 **미스캔 `checkMissedScan`** 등 핫 패스부터. **부분 반영**: `getMergedAttendanceRecordForAutoAbsentSlot`(자동결석 보정 슬롯당 조회 병합). **전면 배치는 미착수**이며 필요 시 Gate A 후 착수 | Network 다발 요청·메인 스레드 부담을 확장 전에 **의도적으로 추적**하기 위해 | `docs/plan.md`, `docs/checklist.md`, 향후 `script.js`·`qr-attendance.js` |
| 2026-03-22 | 전문가 합의(프론트/CSS): QR 스캔 `#qr-scan-page.auth-page`에 **`display: flex !important`**가 있으면 닫기 시 **`style.display='none'`이 CSS에 밀려 무시**되어 오버레이·카메라가 남을 수 있다(콘솔은 `[closeQRScanPage]`만 찍힘). **`display` 규칙에서 `!important` 제거**하고, 열기/닫기/`ensureQrScanFullyClosed`는 **`setProperty('display', flex|none, 'important')`로 통일**. **전문가 합의(프론트/성능)**: `autoMarkAbsentForPastSchedules`가 슬롯마다 출석 **2회 조회**하며 Network에 `attendance_records?student_id=…`가 연속으로 나올 수 있어, **`getMergedAttendanceRecordForAutoAbsentSlot`로 1회 병합 조회**해 메인 스레드·네트워크 부담을 줄인다 | 사용자 재현(닫아도 QR UI 잔류·Network 다발 요청) | `style.css`, `qr-attendance.js`, `script.js` |
...
| 2026-03-22 | 전문가 합의(프론트/아키텍처): `#qr-scan-page`는 `pageStates`(auth/teacher-select/main)에 포함되지 않아 **`navigateToPage('MAIN_APP')`만으로는 숨겨지지 않는다**. 이전에 QR이 열린 뒤 메인으로 오면 z-index·카메라가 유지될 수 있어, **`ensureQrScanFullyClosed`** 를 페이지 전환·`setCurrentTeacher` 시작 시 호출한다. html5-qrcode `stop()` 실패 대비 **`video` MediaStream 트랙 `stop()`** 보조. 닫기 PIN은 **관리자 검증 실패 시 현재 교사 PIN** 재시도로 운영 혼동을 줄인다 | 사용자 재현(입장 시 카메라·닫기 PIN 무반응) | `script.js` `navigateToPage`, `setCurrentTeacher`, `qr-attendance.js` |
| 2026-03-22 | 전문가 합의(프론트/운영): 선생님 선택 **QR 비밀번호 모달**은 입장 인증용이며, 성공 시 **`openQRScanPage` 자동 호출은 제거**한다. 메인 진입 후 사용자가 QR 메뉴를 눌렀을 때만 카메라를 연다. **닫기** 시 `stopQRScannerForModeChange`를 **`await`** 해 스트림 해제 경쟁을 줄인다 | 사용자 보고(비밀번호만 입력했는데 카메라 켜짐·닫기로 안 꺼짐) — 원인 `confirmQRPassword`/`showQRPasswordModal` 자동 오픈 + 비동기 stop 미대기 | `qr-attendance.js` |
| 2026-03-22 | 전문가 합의(UIUX/프론트): 브라우저 **전체화면** 시 QR 스캔 **닫기 버튼은 숨기고**, 전체화면 해제 후에만 노출한다(`fullscreenchange`·페이지 오픈 시 동기화). 학생·외부인이 전체화면 중 종료를 누르기 어렵게 하려는 운영 목적 | 사용자 요청 | `qr-attendance.js`, `style.css`, `index.html` |
| 2026-03-22 | 전문가 합의(UIUX/프론트): QR 스캔 헤더는 `#qr-scan-page`가 safe-area만 두면 **데스크톱에서 상단 패딩이 0**에 가까워 제목·닫기가 붙어 보인다 → `calc(env(safe-area) + clamp)`로 최소 여백을 둔다. **전체화면·스와이프 안내 문구**는 노출면·혼선을 줄이기 위해 **삭제**한다 | 사용자 보고(상단 밀착·안내 문구 불필요) | `style.css`, `mobile.css`, `index.html` |
...
| 2026-03-22 | 전문가 합의(교육운영/UIUX): 출석 이력의 **처리방식**은 QR·번호입력·선생님 체크·임시 체크 네 가지로만 노출하고, **자리 확인**은 별도 배지가 아니라 선생님 체크 라벨에 흡수(`선생님 체크 · 자리확인`). **처리시간·인증시간 수정**은 원장(관리자 앱 로그인, `current_user_role=admin`)만 허용해 감사 가능성을 유지한다. 임시 체크는 **빨간 테두리**로 시각 강조해 선생님이 누락 일정·후속 정리를 놓치지 않게 한다 | 운영자 요청(라벨 단순화·권한 분리·임시 출석 가시성)과 화면 혼잡(자리확인 중복) 완화 | `qr-attendance.js`, `script.js`, `style.css` |
| 2026-03-22 | 전문가 합의(프론트/DB운영): 날짜별 커스텀 일정은 **하루 1행 UNIQUE**가 아니라 **동일 날짜 다행 + `font_size`**로 확장한다. 기존 로컬 단일 객체는 읽기 시 **배열로 정규화**해 호환한다 | 운영에서 같은 날 여러 안내(예비군+시험 등)와 표시 크기 조절 요구가 있고, DB 제약이 곧 앱 한계이기 때문 | `SUPABASE_HOLIDAYS_MULTI_FONT_20260322.sql`, `holidays`, `database.js`, `script.js`, `index.html` |
| 2026-03-22 | 전문가 합의(프론트/운영): 관리자 로그인 직후 지연은 **순차 네트워크**(학생→출석)와 **선생님 `teacher_role` 중복 조회**, 빈 화면 대기가 겹친 결과로 보는 것이 합리적 → **학생·출석 병렬 로드**, 목록에 역할 있으면 **재조회 생략**, 전환 시 **로딩 오버레이**로 체감 완화 | 사용자 체감 개선과 데이터 정합성(병렬 fetch는 동일 결과 병합)의 균형 | `script.js` `loadAndCleanData`/`setCurrentTeacher`, `auth.js` `showMainApp` |
| 2026-03-22 | 전문가 합의(백엔드/보안/프론트): **2차 정적 스캔** — `eval`/`new Function` 미사용, `grading-server`는 `SUPABASE_JWT_SECRET`이 비어 있으면 `/api` JWT 미들웨어가 사용자를 강제하지 않음 → **운영은 JWT 시크릿·CORS_ORIGINS·서비스 롤 키 환경변수를 필수로 관리**. 프론트는 `innerHTML` 다수 사용으로 **DB/사용자 입력이 템플릿에 끼어 들어가는 경로**는 RLS와 별도로 XSS(이스케이프/DOMPurify 등) 점검 대상 | 전 라인 수동 리뷰 대신 고위험 패턴·백엔드 인증 조건을 빠르게 스캔해 회귀 신호를 문서화 | `docs/plan.md` 변경 이력, `grading-server/auth.py`, `main.py`, 주요 `*.js` |
| 2026-03-22 | 전문가 합의(QA/백엔드/프론트): **자동 회귀 묶음**을 `문법검사(node --check)` + **`grading-server` 바이트코드 컴파일** + **충돌 마커 스캔** + **로컬 정적 서버 + Playwright 격리 러너** + **핵심 경로 HTTP 200 스모크**로 고정하면, 코드 변경 없는 “전수 코드 리뷰” 요청에도 재현 가능한 품질 게이트를 제공할 수 있다 | 사용자 요청(전문가 관점 전체 검사 + E2E/스모크)을 **동일 작업일·동일 기준**으로 문서에 남기고, E2E는 **127.0.0.1:8000 서버 선기동**이 필수임을 명시해 오탐(연결 거부)을 줄이기 위함 | `docs/plan.md`, `docs/checklist.md`, `tmp-e2e-runner/`, 로컬 검증 절차 |
| 2026-03-22 | 커밋 시 문서 4종을 자동 연동 업데이트한다 | 작업 중 수동 문서 기록 누락과 문서 간 불일치를 방지하기 위해 | CREATE_EVALUATIONS_TABLE.sql, HOMEWORK_SETUP.sql, SUPABASE_ATTENDANCE_RLS_WRITE_HOTFIX_20260321.sql, SUPABASE_AUTH_CODE_PUBLIC_ACCESS_RESTORE.sql, SUPABASE_COMPLETE_SETUP.sql 외 64개 |
| 2026-03-22 | 사용자 실기기 확인: 메인 진입 후 **오늘 일정·출석 저장·메인↔숙제↔채점↔학부모 포털** 스모크 **PASS** | 로그인/입장·Supabase 설정 이후 핵심 운영 동선이 재현 가능함이 확인되어 회귀 기준선으로 고정 | `docs/checklist.md` Gate C·페이지 연결 항목 |
| 2026-03-22 | 전문가 합의(프론트엔드/백엔드): `sb_publishable_…` 형태 anon 키는 **JWT가 아닐 수 있어** Edge 게이트웨이 `Authorization: Bearer`에 넣으면 **401**이 난다. PIN 검증은 함수 내부에서 처리하므로 **대시보드에서 JWT 검증 OFF** + 클라이언트는 **세션 access_token만 Bearer**로 사용한다 | invoke+fetch 모두 401이면 코드만으로는 한계가 있어 **운영 설정(대시보드)** 과 병행한다 | `js/invoke-verify-teacher-pin.js`, `docs/SUPABASE_VERIFY_TEACHER_PIN_DASHBOARD.md` |
| 2026-03-22 | 전문가 합의(프론트엔드/백엔드): `functions.invoke`가 401이면 **함수 코드가 아닌 게이트웨이/SDK 경로**일 수 있어, 동일 본문으로 **`fetch` 재시도**(`apikey`+`Bearer` 세션 우선)를 둔다. 포털/채점은 `supabase-config.js`를 쓰지 않으므로 **`invokeVerifyTeacherPin(client, body, { url, anonKey })`** 로 URL/키를 넘긴다 | 단일 원인 가정으로 Edge Function만 수정하면 현장에서 401이 남을 수 있어 **클라이언트 회복 경로**를 병행한다 | `js/invoke-verify-teacher-pin.js`, `index.html`, `parent-portal/index.html`, `grading/index.html`, `script.js`, `parent-portal/report.js` |
| 2026-03-22 | 전문가 합의(보안/백엔드): `verify-teacher-pin`은 Edge Function 게이트웨이에서 **`verify_jwt` 기본값 true**이면 유효 JWT 없이(또는 만료 세션) **함수 본문 전에 401**이 난다. PIN 검증은 함수가 service role로 처리하므로 `config.toml`에서 **`verify_jwt = false`** 로 두고, 비즈니스 실패는 HTTP 200+JSON으로 구분한다 | 콘솔에 `401 Unauthorized`만 반복되면 “비번 틀림”과 “세션 만료”를 사용자가 구분하기 어렵다 | `supabase/config.toml`, `npx supabase@latest functions deploy verify-teacher-pin` |
| 2026-03-22 | 전문가 합의(보안/프론트엔드): 원장(관리자) 앱 로그인은 `signIn`·세션 복원(`initializeAuth`)·`showMainApp`에서 **`users.role === 'admin'`** 을 일관되게 검증한다. `users` 조회 실패 시 “진행”하면 비관리자·RLS 오류 시 계정이 열릴 수 있어 **조회 실패=로그인 거부**로 둔다. `confirmAdminVerifyAndChangeRole`는 **재인증 이메일=현재 세션 이메일**·**`users.role=admin`**·**user id=current_owner_id**로 계정 교체·권한 오남용을 방지한다. `backToAdminLogin`은 `remember_login`까지 제거해 다른 원장 계정 혼선을 줄인다 | 단일 관리자 앱인데 인증 경로마다 검증이 다르면 우회·혼선이 생긴다 | `auth.js`, `js/teacher-manage.js`, `parent-portal/report.js`, `homework/index.html`, `script.js` |
| 2026-03-22 | 전문가 합의(프론트엔드/백엔드): `verify-teacher-pin`의 **비즈니스 실패(필드 누락·교사 없음·소유권·관리자 필요·PIN 불일치)** 는 **HTTP 401/403/404 대신 200 + `{ ok: false, error?: ... }`** 로 응답한다. `supabase.functions.invoke()`는 비-2xx에서 `error`만 채우고 `data`가 비는 경우가 많아, PIN 오류를 401로 쓰면 콘솔/디버깅에서 **게이트웨이 인증 실패와 동일하게 보이는** 혼선이 생긴다 | 앱 수준 결과는 항상 JSON으로 파싱 가능하게 하고, 진짜 예외만 500으로 남긴다 | `supabase/functions/verify-teacher-pin/index.ts`, 배포 후 `script.js` 등 `verifyTeacherPinWithServer` 호출부 |
| 2026-03-22 | 전문가 합의(프론트엔드/운영): `verify-teacher-pin` CORS는 OPTIONS를 **200 + 본문 `ok`** 로 응답하고 `Allow-Headers`에 `x-supabase-api-version`, `prefer`를 포함한다. Windows에서는 전역 `supabase` 미설치가 흔하므로 **`npx supabase@latest functions deploy verify-teacher-pin`** 로 배포한다. 콘솔에 보이는 `SUPABASE_URL`이 `...omjmytfekcgqu...`와 다르면(문자 누락/오타) 다른 호스트로 요청되어 preflight가 비정상·CORS 오류로만 보일 수 있다 | 프리플라이트가 2xx가 아니면 브라우저가 POST를 막는다. 함수 미배포·URL 오타·OPTIONS 응답 형식은 동일 증상으로 혼동되므로 점검 순서를 고정한다 | `supabase/functions/verify-teacher-pin/index.ts`, `supabase-config.js`, 배포 명령(npx) |
| 2026-03-08 | 전문가 합의(프론트엔드/백엔드): `verify-teacher-pin`은 CORS preflight에 `Access-Control-Allow-Methods`(POST, OPTIONS)·`Max-Age`를 포함한다(후속: OPTIONS는 200+`ok`로 통일). 배포 미반영 시에는 404/5xx로 preflight가 실패해 브라우저가 “CORS”로만 보고한다 | 로컬(`127.0.0.1:5500`)에서 `functions.invoke` 시 브라우저가 OPTIONS를 먼저 보내는데, 메서드 헤더 누락 또는 비정상 상태코드면 “preflight가 HTTP ok가 아님”으로 차단된다. 코드 보강 후에도 **반드시 Edge Function 재배포**가 필요하다 | `supabase/functions/verify-teacher-pin/index.ts`, Supabase CLI `functions deploy` |
| 2026-03-21 | 전문가 합의(보안/프론트엔드/백엔드): PIN 검증은 클라이언트 `pin_hash` 비교를 중단하고 `verify-teacher-pin` Edge Function 서버 검증으로 통일한다 | 클라이언트가 `teachers`에서 `pin_hash`를 받아 직접 비교하면 해시 노출 및 오프라인 대입 위험이 남으므로, 검증 책임을 서버로 이동하고 클라이언트에는 성공/실패 결과만 반환해야 노출면을 줄일 수 있기 때문 | supabase/functions/verify-teacher-pin/index.ts, script.js, qr-attendance.js, parent-portal/report.js, grading/index.html |
| 2026-03-21 | 전문가 합의(보안/백엔드): 재강화 3단계는 `students_public_read`, `homework_schedules_read` 2개 공개 읽기 정책만 포털 코드 기반 최소권한으로 축소하고, 동일 범위 롤백 SQL을 세트로 제공한다 | 일정/포털 조회와 직결된 정책이라 범위를 넓게 건드리면 회귀 위험이 커서, 핫픽스로 완화된 2개 정책만 독립 축소/복귀 가능하게 분리해야 안전하게 보안수준을 끌어올릴 수 있기 때문 | SUPABASE_REHARDEN_STAGE3_STUDENTS_SCHEDULES_APPLY_20260321.sql, SUPABASE_REHARDEN_STAGE3_STUDENTS_SCHEDULES_ROLLBACK_20260321.sql |
| 2026-03-21 | 재강화 2단계는 attendance_records 앱 owner read/write 4개 정책만 auth.uid() 기반으로 축소하고, 같은 범위의 롤백 SQL을 함께 제공한다 | 출석 저장은 운영 핵심이므로 전체 정책을 한 번에 조정하면 재손상 위험이 크다. 정책 4개 단위로 축소/복귀를 분리해야 안전하게 보안 강화를 진행할 수 있기 때문 | SUPABASE_REHARDEN_STAGE2_ATTENDANCE_APPLY_20260321.sql, SUPABASE_REHARDEN_STAGE2_ATTENDANCE_ROLLBACK_20260321.sql |
| 2026-03-21 | 재강화 1단계는 `teachers_public_read` 단일 정책만 축소하고, 실패 시 즉시 복귀 가능한 롤백 SQL을 같은 배치로 제공한다 | 긴급복구 직후 운영 안정성을 유지하면서 보안 권한을 다시 줄이려면 변경 범위를 최소화하고 즉시 복구 경로를 보장해야 재손상 위험을 낮출 수 있기 때문 | SUPABASE_REHARDEN_STAGE1_APPLY_20260321.sql, SUPABASE_REHARDEN_STAGE1_ROLLBACK_20260321.sql |
| 2026-03-21 | 긴급복구 직후 스모크 3종(일정 추가/출석 변경/미처리 전환) PASS를 기준선으로 고정하고, 다음 변경은 재강화 SQL을 적용/롤백 세트로만 진행한다 | 현재 운영이 정상 복구된 상태에서 즉시 추가 변경을 하면 재손상 위험이 커서, 기준선을 먼저 확정한 뒤 작은 단위 재강화로 전환해야 안전하기 때문 | docs/plan.md, docs/context.md, docs/checklist.md, docs/enterprise_workflow.md, Supabase SQL 운영 절차 |
| 2026-03-21 | attendance_records 401/42501 장애는 RLS write 정책 차단으로 확정하고, 학생 소유(owner) 일치 조건의 앱 read/write 정책을 긴급 추가한다 | 콘솔에 `new row violates row-level security policy for table attendance_records`가 반복되어 출석 저장이 실제 실패하고, 일정 생성/자동결석/미처리 정리 전반이 연쇄 실패하기 때문 | SUPABASE_ATTENDANCE_RLS_WRITE_HOTFIX_20260321.sql, qr-attendance.js, script.js |
| 2026-03-20 | 일정 DB 저장 배치 호출은 요청 전 세션 유효성을 먼저 확인해 401 네트워크 요청을 선차단한다 | 현재는 세션 만료 상태에서도 바로 upsert를 시도해 401이 발생하고, 사용자 입장에서는 일정 생성 성공/실패가 혼동되므로 요청 전 차단으로 오류 체감을 낮춰야 하기 때문 | database.js(`saveScheduleToDatabase`, `saveSchedulesToDatabaseBatch`), script.js 일정 생성 플로우 |
| 2026-03-20 | 일정 생성은 로컬 반영과 DB 동기화 결과를 분리해 사용자에게 안내하고, DB 실패가 나도 성공 토스트 누락/미노출이 발생하지 않게 예외를 흡수한다 | 현재 장애는 첫 저장에서 로컬 일정은 생성되지만 DB 401 예외가 상위로 전파돼 완료 토스트가 사라져 사용자가 "안 됐다"고 재시도하며 중복 오해가 생기는 흐름이기 때문 | script.js(`_generateScheduleCore`), database.js 연계 호출 |
| 2026-03-20 | 메인 일정 미노출 장애는 teachers 공개읽기 정책 축소와 현재 클라이언트 조회 경로 충돌로 판정하고, 읽기 경로만 임시 완화해 즉시 복구한다 | 사용자 운영이 중단된 상태에서는 표시 경로를 먼저 복구해 업무 연속성을 확보해야 하며, 이후 PIN 서버검증 전환과 함께 정책을 재강화하는 2단계 접근이 안전하기 때문 | SUPABASE_TEACHERS_PUBLIC_READ_HOTFIX_20260320.sql, docs/plan.md, docs/checklist.md |
| 2026-03-20 | Supabase SQL Editor에 바로 실행 가능한 teachers 정책 보강/검증 통합 스크립트를 추가한다 | 운영자가 코드와 SQL 반영 상태를 한 파일에서 바로 적용/검증해야 오적용과 누락을 줄일 수 있기 때문 | SUPABASE_TEACHERS_POLICY_HARDEN_AND_VERIFY.sql |
| 2026-03-20 | 연속 작업은 진행현황을 단계별로 사용자에게 공유하고 같은 사이클에 docs를 즉시 동기화한다 | 다단계 작업에서 상태 공유가 늦어지면 확인 타이밍과 작업 기록이 어긋나 재작업 비용이 커지기 때문 | docs/plan.md, docs/context.md, docs/checklist.md, docs/enterprise_workflow.md |
| 2026-03-20 | RLS 잔여 과허용을 teacher 정책까지 축소하고 클라이언트 로그의 해시 노출을 제거한다 | 남아있던 teacher 공개정책 과범위와 콘솔 해시 노출은 권한오남용/정보노출 위험을 키우므로 기능 영향이 작은 범위부터 즉시 차단해야 하기 때문 | SUPABASE_COMPLETE_SETUP.sql, script.js |
| 2026-03-20 | 중요작업 우선순위에 따라 RLS 과허용 정책과 Edge Function 무검증 업데이트를 1차 차단한다 | 스크린샷 기반 보안 재점검에서 데이터 노출/권한오용 위험이 가장 큰 항목이 정책 과허용과 서비스키 무검증 업데이트로 확인되었기 때문 | SUPABASE_COMPLETE_SETUP.sql, HOMEWORK_SETUP.sql, CREATE_EVALUATIONS_TABLE.sql, supabase/functions/exchange-google-token/index.ts, supabase/functions/upload-homework/index.ts |
| 2026-03-20 | 에러메시지 축소, RLS 최소권한, 서버검증, 관리자 라우트 보호를 보안 1순위로 확정 | 첨부 이미지의 핵심 체크포인트와 현재 코드/SQL 점검 결과가 동일 취약 축에서 고위험 항목을 확인했기 때문 | docs/plan.md, docs/context.md, docs/checklist.md, docs/enterprise_workflow.md, SUPABASE_COMPLETE_SETUP.sql, CREATE_EVALUATIONS_TABLE.sql, HOMEWORK_SETUP.sql, supabase/functions/*, script.js, grading/index.html |
| 2026-03-20 | 중요 작업은 전문가 토론 결과를 선기록한 뒤 구현을 진행한다 | 고위험 변경에서 판단 누락을 줄이고 대응 일관성을 확보하기 위해 | docs/enterprise_workflow.md, docs/plan.md, docs/context.md, docs/checklist.md |
| 2026-03-20 | pre-commit 훅으로 docs 4종 기록을 커밋 시 자동 연동한다 | 명령 수동 실행 없이도 문서 누락/불일치를 원천 차단하기 위해 | docs/plan.md, docs/context.md, docs/checklist.md, docs/enterprise_workflow.md, .git/hooks/pre-commit, qa-artifacts |
| 2026-03-20 | docs 4종 기록을 단일 명령으로 연동 업데이트한다 | 문서별 수동 입력 누락과 불일치를 줄여 작업 이력 신뢰도를 높이기 위해 | docs/plan.md, docs/context.md, docs/checklist.md, docs/enterprise_workflow.md, qa-artifacts |
| 2026-03-20 | 전문가 합의(프론트엔드/운영QA): 엔터프라이즈 플레이북 문서는 수동 관리가 아니라 날짜 동기화/작업로그 추가를 스크립트로 자동화한다 | 프로세스 문서는 누락 없이 최신 상태를 유지해야 가치가 있는데, 수동 갱신만으로는 작업 사이클이 길어질수록 누락 가능성이 높기 때문 | `qa-artifacts/sync_doc_dates.py`, `qa-artifacts/append_enterprise_log.py`, `docs/enterprise_workflow.md` |
| 2026-03-20 | 전문가 합의(프론트엔드/백엔드/운영QA): 이 앱의 개발 절차를 엔터프라이즈 경량 표준(Gate A~E, Risk-based 테스트, PRD-lite)으로 고정하고 모든 패치에 적용한다 | 기능 추가 속도보다 회귀 방지와 장애 복구 속도가 운영 품질에 더 큰 영향을 주고, 현재 프로젝트는 페이지 연동/인증코드/Supabase 정책이 서로 연결되어 있어 변경 절차 표준화가 필수이기 때문 | `docs/enterprise_workflow.md`, `docs/plan.md`, `docs/checklist.md`, 전체 기능 패치 사이클 |
| 2026-03-20 | 전문가 합의(프론트엔드/DB운영/운영QA): 장애 대응은 `코드 vs Supabase vs 외부플랫폼` 3분류를 먼저 확정하고, 분류 근거가 확인되기 전에는 수정 범위를 고정하지 않는다 | 원인 미확정 상태에서 코드·SQL을 동시에 손대면 오진으로 엉뚱한 부분을 수정하게 되고, 실제 장애 복구 시간이 더 길어지기 때문 | `docs/plan.md`, `docs/checklist.md`, 모든 장애 대응 사이클 |
| 2026-03-20 | 전문가 합의(DB운영/프론트엔드): 학부모 포털 출결 미노출은 `attendance_records` anon SELECT 정책 부재/제한을 우선 원인으로 보고 별도 복구 SQL로 분리 대응한다 | 학생 조회(`students`)는 복구됐지만 출결 조회(`attendance_records`)가 계속 빈 결과면 포털 달력은 0건으로 표시되므로, 인증코드 정책과 출결정책을 분리 점검해야 재발 원인을 빠르게 고정할 수 있기 때문 | `SUPABASE_PARENT_PORTAL_ATTENDANCE_READ_RESTORE.sql`, `parent-portal/report.js` |
| 2026-03-20 | 전문가 합의(프론트엔드/운영QA): 포털 인증 장애 재발 방지를 위해 `parent-portal`/`homework`에도 메인과 동일한 env 로더 체인을 적용한다 | 메인 페이지는 env 로더를 통해 런타임 설정을 읽지만 포털 페이지는 하드코딩 fallback에 의존하면 프로젝트 연결이 어긋날 수 있어, 동일 설정체인으로 통일해야 환경별 불일치를 줄일 수 있기 때문 | `parent-portal/index.html`, `homework/index.html` |
| 2026-03-20 | 전문가 합의(프론트엔드/DB운영): 인증코드 조회는 오입력과 정책 차단을 구분해 안내해야 하므로, 미매칭 시 `students` public 조회 프로브로 진단 메시지를 분리한다 | 현재 사용자에게는 모두 `유효하지 않은 인증코드`로 보이지만 실제 원인은 RLS/조회권한 차단일 수 있어, 원인 구분 메시지가 있어야 운영자가 SQL 복구를 빠르게 수행할 수 있기 때문 | `homework/index.html`, `parent-portal/report.js`, `SUPABASE_AUTH_CODE_PUBLIC_ACCESS_RESTORE.sql` |
| 2026-03-20 | 전문가 합의(프론트엔드/운영QA/DB운영): 숙제제출+학부모포털 인증코드 장애는 Railway 과금 상태와 분리해 Supabase `students` 조회/RLS 정책 체인으로 우선 진단하고, 정책 복구 SQL을 별도 파일로 고정한다 | 두 포털 모두 인증코드 조회는 직접 `students`를 읽는 구조라 Railway 백엔드 장애와 원인 경로가 다르며, 운영에서 빠르게 재복구하려면 정책 점검/복구/재점검을 일괄 수행 가능한 실행 파일이 필요하기 때문 | `homework/index.html`, `parent-portal/report.js`, `SUPABASE_AUTH_CODE_PUBLIC_ACCESS_RESTORE.sql` |
| 2026-03-20 | 전문가 합의(프론트엔드/운영UX): 숙제 제출 `student_code` 조회도 학부모 포털과 동일한 코드 정규화(공백/하이픈/대소문자/전각 흡수) + 다단계 조회로 통일한다 | 숙제 페이지는 기존에 완전일치 단일 조회라 입력 포맷 변형에서 실패율이 높고, 포털별 조회 정책이 다르면 현장에서는 동일 코드 정책으로 인지해 "한쪽만 실패" 혼선이 반복되기 때문 | `homework/index.html`(`normalizeHomeworkPortalCode`, `findStudentsByStudentCode`, `handleSearch`) |
| 2026-03-20 | 전문가 합의(프론트엔드/운영QA/보안): 학부모 포털 인증코드는 입력/저장 포맷 차이(공백, 하이픈, 대소문자, 전각 문자)를 흡수하는 정규화 비교로 조회·검증한다 | 기존 단일 `eq(parent_code)` 완전일치는 현장 복붙/메신저 전달 과정의 포맷 변형에서 오탐(`유효하지 않은 인증코드`)을 유발할 수 있어, 조회 단계부터 정규화된 동일성 판정을 적용해야 사용자 실패율을 줄일 수 있기 때문 | `parent-portal/report.js`(`normalizeParentPortalCode`, `findStudentByParentCode`, `handleSearch`, `handleParentAuth`) |
| 2026-03-20 | 전문가 합의(프론트엔드/운영QA): 패치 작업마다 핵심 페이지 전환 링크(메인, 숙제 제출, 채점, 학부모 포털)를 상시 회귀 점검 항목으로 유지한다 | 정적 페이지 구조에서는 기능 패치가 직접 관련 없어 보여도 상대경로/열기 URL 변경으로 다른 페이지 진입이 깨질 수 있어, 릴리즈 안정성을 위해 페이지 간 연결 점검을 기본 게이트로 고정해야 하기 때문 | `index.html`, `js/payment.js`, `homework/*`, `grading/*`, `parent-portal/*`, `docs/checklist.md` |
| 2026-03-20 | 전문가 합의(프론트엔드/운영UX): 라이브서버에서 학부모 포털은 디렉터리 URL을 항상 trailing slash(`.../parent-portal/`) 형태로 열도록 표준화한다 | 일부 정적 서버는 폴더 경로를 슬래시 없이 열 때 인덱스 파일 리졸브가 실패하거나 상대경로 자산 로딩이 깨질 수 있어, 버튼 열기 경로를 정규화해 접속 실패를 줄여야 하기 때문 | `js/payment.js`(`openParentPortal`) |
| 2026-03-20 | 전문가 합의(프론트엔드/권한): 관리자 비밀번호 인증이 성공하면 일정 편집은 슬롯 소유 교사와 무관하게 일정/출석 수정·삭제를 일관 허용하고, 인증 상태를 짧은 세션으로 유지한다 | 사용자 운영 요구가 "관리자 인증 후 전체 시간표 수정 가능"으로 명확하며, 기존 단일 `teacher_id` 기준 삭제는 교차 편집 시 레거시 owner 키 불일치로 일정 박스 잔존을 유발할 수 있어 owner 후보군 기반 삭제와 세션형 권한이 필요하기 때문 | `script.js`(`verifyScheduleEditPermission`, `openAttendanceModal`, `deleteSingleSchedule`), `qr-attendance.js`(`updateAttendanceStatusFromHistory`), 일정관리 119차 |
| 2026-03-18 | 전문가 합의(교육운영/프론트엔드): 일정 등록 기본 수업시간은 학년 분기 없이 100분으로 통일한다 | 사용자 운영 정책이 중/고 모두 100분으로 변경되어 기존 중등 90분 자동 적용은 실제 운영과 불일치하고, 등록 단계에서 수동 수정 누락 리스크를 키우기 때문 | `script.js`(`updateDurationByGrade`, 등록 모달 기본값), `index.html`(`sch-duration-min` 기본값), 일정관리 118차 |
| 2026-03-15 | 전문가 합의(UIUX/프론트엔드): `원장 입력` 버튼은 hover 전에도 즉시 인지 가능해야 하므로 기본 텍스트를 고정 대비 색상으로 지정한다 | 사용자 피드백에서 `원장 입력`이 마우스 오버 전 잘 보이지 않는 문제가 확인되었고, 버튼 배경이 밝은 상태에서 테마 텍스트 변수가 밝게 해석되는 경우 대비가 무너질 수 있기 때문 | `style.css`(`.pay-ledger-btn`), 수납관리 117차 |
| 2026-03-15 | 전문가 합의(UIUX/프론트엔드): 일/월마감 요약 카드의 텍스트는 테마 변수 대신 카드 배경 대비에 맞춘 고정 색상으로 표시한다 | 사용자가 `오늘 수납액/오늘 수납건수/월 미수금/미확인입금` 텍스트가 지나치게 희게 보여 읽기 어렵다고 보고했고, 요약 카드는 배경이 밝게 고정되어 있으므로 다크 테마 변수 텍스트를 그대로 쓰면 대비가 떨어지기 때문 | `style.css`(`.pay-close-label`, `.pay-close-card strong`), 수납관리 116차 |
| 2026-03-15 | 전문가 합의(UIUX/프론트엔드): 수납 학생카드 상세의 우측 값 텍스트는 테마 변수 대신 배경 고정 대비 색으로 지정한다 | 다크 테마에서 상세 행 배경은 밝게 유지되는데 우측 값이 테마 텍스트(밝은색)를 따라가 과도하게 희게 보여 가독성이 떨어진다는 사용자 피드백이 확인되어, 정보 가독성을 우선해 행 단위 대비를 고정해야 하기 때문 | `style.css`(`.pay-ledger-row span/strong`), 수납관리 115차 |
| 2026-03-15 | 전문가 합의(수납/프론트엔드/데이터정합성): 학생 로드 시 수납 원장(`payments`)은 DB 기본값으로 덮어쓰지 말고 로컬 원장 캐시를 병합해 보존한다 | 사용자 실기기에서 수납 금액/날짜 수정 후 새로고침 시 값이 유실되었고, 원인이 `loadAndCleanData`의 `payments: {}` 초기화로 확인되어 원장 데이터 보존 규칙을 명시적으로 적용할 필요가 있기 때문 | `script.js`(`loadAndCleanData`), `js/payment.js`(`savePaymentLedger`), 수납관리 114차 |
...
| 2026-03-09 | 66차 실기기 회귀는 운영자가 즉시 입력 가능한 고정 템플릿(요약/경로별/실패필수기록)으로 수집 | 경로가 5개(QR/번호/수동/임시/재석확인)라 구두 보고만으로는 누락이 잦아 PASS/FAIL 근거를 구조화해야 하기 때문 | `docs/checklist.md`, 66차 검증 품질 |
| 2026-03-09 | 66차 운영 SQL 반영 여부는 수동 콘솔 확인만 기다리지 않고 API 컬럼 조회(200)로 즉시 판정하는 보조 검증을 채택 | 실제 운영에서 컬럼 사용 가능 여부를 빠르게 확인해 다음 단계(실기기 메타 정합성)로 지체 없이 넘어가기 위해서 | `qa-artifacts/verify_attendance_meta_columns.py`, `docs/plan.md`, `docs/checklist.md` |
| 2026-03-09 | 잔여 회귀 1,2(공휴일 키 재발/메인 백그라운드 동기화·툴팁 정확성)는 사용자 실기기 PASS를 기준으로 마감하고, 다음 우선순위를 운영 SQL 적용(66차)으로 이동 | 핵심 UI/접근 리스크가 해소되어 남은 구조 리스크는 DB 메타 정규화 적용/검증 쪽이 우선이기 때문 | `docs/plan.md`, `docs/checklist.md`, 다음 작업 우선순위 |
| 2026-03-09 | 문서 기준일은 수동 입력이 아니라 자동 스크립트(`python qa-artifacts/sync_doc_dates.py`)로 맞추는 방식으로 고정 | 다음 작업일에도 과거 날짜를 반복 저장하는 실수를 방지하고 기준일 누락을 구조적으로 줄이기 위해서 | `qa-artifacts/sync_doc_dates.py`, `.cursor/rules/workflow-doc-driven.mdc`, `docs/*` |
| 2026-03-09 | 문서 기준일을 당일(2026-03-09)로 일괄 갱신하고 날짜 드리프트 점검을 상시 우선으로 유지 | 작업 당일 날짜가 누락되면 인계/우선순위 판단이 어긋나므로, 작업 시작 시점에 먼저 고정해야 하기 때문 | `docs/plan.md`, `docs/context.md`, `docs/checklist.md` |
| 2026-03-08 | 실기기 회귀 1순위 3건을 우선 마감(PASS)하고 다음 잔여 회귀를 백그라운드 동기화/툴팁 정확성으로 축소 | 사용자 실기기 확인에서 묶음 인증/메인 진입/초기 배지가 모두 정상으로 확인되어 핵심 운영 리스크를 먼저 닫을 수 있기 때문 | `docs/plan.md`, `docs/checklist.md`, 다음 검증 우선순위 |
| 2026-03-08 | 숙제 제출 긴급 장애 대응 시 우선 확인 URL을 `https://jjyown.github.io/academy_manager/homework/`로 고정 | 하이픈 경로(`academy-manager/homework`)와 Railway 경로(`/homework`)는 404라 모바일에서 접근불가로 보일 수 있기 때문 | 운영 안내 URL, 사용자 공지 문구 |
| 2026-03-08 | 학부모 포털 긴급 장애 대응 시 우선 확인 URL을 `https://jjyown.github.io/academy_manager/parent-portal/`로 고정 | 기존 안내 링크의 하이픈 경로(`academy-manager`)가 404여서 모바일에서 `사이트에 접근할 수 없음`으로 해석되는 오탐이 발생했기 때문 | `README.md`, 운영 안내 URL |
| 2026-03-08 | 런타임/문서 파일의 머지 충돌 마커를 HEAD 기준으로 우선 정리 | 충돌 마커가 남아 있으면 프론트 스크립트 파싱이 실패해 기능 검증 자체가 불가능하기 때문 | `script.js`, `qr-attendance.js`, `index.html`, `docs/*`, `.cursor/rules/workflow-doc-driven.mdc` |
