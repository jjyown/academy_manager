...
| 2026-03-30 | **숙제 제출 Edge 401**: `upload-homework`가 Bearer JWT(원장)만 인정해 학생 포털(무세션)에서 401. **대응**: Form `student_code` + 서버 정규화 일치로 본인 검증, 원장 JWT 경로 병행 | 학생 숙제 제출 복구 | `upload-homework/index.ts`, `homework/index.html` |
...
| 2026-03-30 | **Drive 중앙 루트 단일화(숙제 관리 우선) + HML 미리보기**: My Drive 최상위 폴더를 조회하되 `resolve_central_root_folder_id`가 검색/생성 시 **`숙제 관리`를 최우선**으로 사용하며, 없으면 레거시/환경값 재사용 없이 **`숙제 관리`를 새로 생성**. HML은 원문 페이지 래스터 없음 → `build_hml_answer_preview_images`로 정답·해설 요약 JPEG 생성 후 `page_images_json`과 동일 경로 업로드. 또한 프론트/백엔드에서 `.hwp`도 HML 계열로 판별해 미리보기 누락을 줄임. 추가로 이번 로그에서 `페이지 이미지: 0장`이 관측되어, Pillow 미설치/미이미지 생성 실패 시에도 placeholder JPEG로 최소 1장 생성하도록 보강 | `과제 관리`/`숙제 관리` 이중 루트·HML 이미지 누락(또는 ‘다른 루트에 업로드’로 인한 미인지) 완화 | `drive.py`, `config.py`, `hml_parser.py`, `answer_keys.py` |
