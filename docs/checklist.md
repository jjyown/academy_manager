...
- [ ] 실기기 회귀: 본인 담당 묶음 일정 클릭 시 관리자 인증 없이 진입되는지 확인
- [ ] 실기기 회귀: 타교사 묶음 일정 클릭 시에는 기존대로 관리자 인증이 뜨는지 확인
- [ ] 실기기 재현 케이스 확인: `육효원/조민준`, `2026-03-03`, `23:30~01:10`, 담당 `전재윤` 클릭 시 관리자 인증 미노출 확인
...
- [ ] 브라우저 강력 새로고침 후 공휴일 경고 로그 미출력 확인
...
- [x] 정적 점검: `ReadLints(script.js)` 오류 없음
- [ ] 실기기/브라우저 재확인: 강력 새로고침 후 공휴일 키 미설정 경고가 재발하지 않는지 확인
...
- [x] 정적 점검: `node --check script.js` 통과
- [x] 정적 점검: `ReadLints(script.js)` 오류 없음
- [ ] 실기기 검증: 선생님 선택 후 메인 캘린더가 체감상 즉시 표시되는지 확인
- [ ] 실기기 회귀: 백그라운드 동기화 완료 후 전체 선생님 보기/자동결석 타이머 동작 이상 없는지 확인
- [ ] 실기기 회귀: 월간 뱃지 인원수/툴팁(학생명·교사명) 표시 정확성 유지 확인
- [ ] 실기기 회귀: `3/3`, `3/5`에서 초기 배지가 오집계(1명) 없이 `집계중 -> 정확명수`로 전환되는지 확인
- [ ] 실기기 회귀: `육효원/조민준`, `23:30~01:10`, 담당 `전재윤` 본인 클릭 시 관리자 인증 미노출 확인
=======
>>>>>>> 0f2aa15acf96d7baccc306f584159e28fe45959e
