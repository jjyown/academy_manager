...
def get_supabase() -> Client:
    """Supabase 싱글톤 클라이언트 (thread-safe 초기화)"""
    global _client
    if _client is None:
        with _lock:
            if _client is None:
...
async def run_query(fn, *args, **kwargs):
    """동기 Supabase 쿼리를 쓰레드 풀에서 실행하여 이벤트 루프 블로킹 방지"""
...
async def get_answer_key(answer_key_id: int) -> dict | None:
    try:
        sb = get_supabase()
        res = await run_query(sb.table("answer_keys").select("*").eq("id", answer_key_id).limit(1).execute)
...
async def upsert_answer_key(data: dict) -> dict:
    sb = get_supabase()
    if data.get("id"):
        res = await run_query(sb.table("answer_keys").upsert(data).execute)
    else:
        teacher_id = data.get("teacher_id")
        title = data.get("title")
        if teacher_id and title:
            existing = await run_query(sb.table("answer_keys").select("id").eq(
                "teacher_id", teacher_id
            ).eq("title", title).limit(1).execute)
            if existing.data:
                data["id"] = existing.data[0]["id"]
                res = await run_query(sb.table("answer_keys").update(data).eq("id", data["id"]).execute)
            else:
                res = await run_query(sb.table("answer_keys").insert(data).execute)
        else:
            res = await run_query(sb.table("answer_keys").insert(data).execute)
...
async def get_assignment(assignment_id: int) -> dict | None:
    try:
        sb = get_supabase()
        res = await run_query(sb.table("grading_assignments").select("*").eq("id", assignment_id).limit(1).execute)
        return res.data[0] if res.data and len(res.data) > 0 else None
    except Exception as e:
...
async def get_assignments_by_teacher(teacher_id: str) -> list[dict]:
    sb = get_supabase()
    res = await run_query(sb.table("grading_assignments").select("*, answer_keys(title, subject)").eq("teacher_id", teacher_id).order("created_at", desc=True).execute)
    return res.data or []
...
async def delete_assignment(assignment_id: int) -> bool:
    sb = get_supabase()
    res = await run_query(sb.table("grading_assignments").delete().eq("id", assignment_id).execute)
...
async def get_student_assigned_key(student_id: int) -> dict | None:
    ...
    try:
        ...
        res = await run_query(sb.table("grading_assignments").select(
            ...
            "assigned_students", "cs", contains_value
...
async def get_student_books(student_id: int) -> list[dict]:
    sb = get_supabase()
    res = await run_query(sb.table("student_books").select(
...
async def add_student_book(student_id: int, answer_key_id: int, teacher_id: str) -> dict:
    sb = get_supabase()
    res = await run_query(sb.table("student_books").upsert(
...
async def remove_student_book(student_book_id: int) -> bool:
    sb = get_supabase()
    res = await run_query(sb.table("student_books").delete().eq("id", student_book_id).execute)
    return bool(res.data)
...
async def create_grading_result(data: dict) -> dict:
    sb = get_supabase()
    res = await run_query(sb.table("grading_results").insert(data).execute)
    return res.data[0] if res.data else {}
...
async def update_grading_result(result_id: int, data: dict) -> dict:
    ...
    res = await run_query(sb.table("grading_results").update(data).eq("id", result_id).execute)
    return res.data[0] if res.data else {}
...
async def get_grading_results_by_student(student_id: int) -> list[dict]:
    sb = get_supabase()
    res = await run_query(sb.table("grading_results").select("*, answer_keys(title, subject)").eq("student_id", student_id).eq("status", "confirmed").order("created_at", desc=True).execute)
    return res.data or []
...
async def create_grading_items(items: list[dict]) -> list[dict]:
    sb = get_supabase()
    res = await run_query(sb.table("grading_items").insert(items).execute)
    return res.data or []
...
async def get_pending_submissions() -> list[dict]:
    """채점 대기 중인 숙제 목록 조회"""
    sb = get_supabase()
    res = await run_query(sb.table("homework_submissions").select(
...
async def get_student(student_id: int) -> dict | None:
    try:
        sb = get_supabase()
        res = await run_query(sb.table("students").select("*").eq("id", student_id).limit(1).execute)
        return res.data[0] if res.data and len(res.data) > 0 else None
    except Exception as e:
...
async def get_students_by_teacher(teacher_id: str) -> list[dict]:
    sb = get_supabase()
    res = await run_query(sb.table("students").select("*").eq("teacher_id", teacher_id).execute)
    return res.data or []
...
async def get_teacher(teacher_id: str) -> dict | None:
    try:
        sb = get_supabase()
        res = await run_query(sb.table("teachers").select("*").eq("owner_user_id", teacher_id).limit(1).execute)
        return res.data[0] if res.data and len(res.data) > 0 else None
    except Exception as e:
...
async def get_teacher_by_id(teacher_table_id: int) -> dict | None:
    try:
        sb = get_supabase()
        res = await run_query(sb.table("teachers").select("*").eq("id", teacher_table_id).limit(1).execute)
        return res.data[0] if res.data and len(res.data) > 0 else None
    except Exception as e:
...
async def upsert_grading_stats(data: dict) -> dict:
    sb = get_supabase()
    res = await run_query(sb.table("grading_stats").upsert(data, on_conflict="teacher_id,answer_key_id,month").execute)
    return res.data[0] if res.data else {}
...
async def create_notification(data: dict) -> dict:
    ...
    try:
        sb = get_supabase()
        res = await run_query(sb.table("notifications").insert(data).execute)
        return res.data[0] if res.data else {}
    except Exception as e:
...
async def get_notifications(teacher_id: str, unread_only: bool = False, limit: int = 50) -> list[dict]:
    ...
    res = await run_query(query.order("created_at", desc=True).limit(limit).execute)
    return res.data or []
...
async def upsert_evaluation(data: dict) -> dict:
    sb = get_supabase()
    res = await run_query(sb.table("evaluations").upsert(data).execute)
    return res.data[0] if res.data else {}
